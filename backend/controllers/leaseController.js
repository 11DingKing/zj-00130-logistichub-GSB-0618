...
const createLease = (req, res) => {
  ...
  if (pricingTiers) {
    if (!Array.isArray(pricingTiers) || pricingTiers.length === 0) {
      return res.status(400).json({ error: "阶梯计价配置必须为非空数组" });
    }
    for (const tier of pricingTiers) {
      if (typeof tier.min !== "number" || typeof tier.unit_price !== "number") {
        return res.status(400).json({
          error: "阶梯计价每档需包含 min(number) 与 unit_price(number)",
        });
      }
      if (
        tier.max !== null &&
        ...
        return res
          .status(400)
          .json({ error: "阶梯计价 max 必须为数字或 null（表示无上限）" });
      }
    }
  ...
  const result = db
    .prepare(
      `
    INSERT INTO leases (merchant_id, location_id, category_id, agreed_capacity, billing_method, unit_price, start_date, end_date, remarks, pricing_tiers)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `,
    )
    .run(
