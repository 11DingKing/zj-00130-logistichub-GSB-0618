...
const createLease = (req, res) => {
  const { locationId, categoryId, agreedCapacity, billingMethod, unitPrice, startDate, endDate, remarks } = req.body;

  if (!locationId || !categoryId || !agreedCapacity || !billingMethod || !unitPrice || !startDate) {
    return res.status(400).json({ error: '库位ID、品类ID、约定容量、计费方式、单价、开始日期为必填项' });
  }
  ...
  const category = db.prepare('SELECT * FROM goods_categories WHERE id = ?').get(categoryId);
  if (!category) {
  ...
  const activeLease = db.prepare('SELECT id FROM leases WHERE location_id = ? AND status = ?').get(locationId, 'active');
  if (activeLease) {
    return res.status(400).json({ error: '该库位已有有效租约' });
  }

  const result = db.prepare(`
    INSERT INTO leases (merchant_id, location_id, category_id, agreed_capacity, billing_method, unit_price, start_date, end_date, remarks)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).run(req.user.id, locationId, categoryId, agreedCapacity, billingMethod, unitPrice, startDate, endDate, remarks);

  db.prepare('INSERT INTO system_logs (user_id, action, module, details) VALUES (?, ?, ?, ?)')
    .run(req.user.id, 'create_lease', 'leases', `申请库位租赁: 库位ID ${locationId}`);
...
const approveLease = (req, res) => {
  ...
  const location = db.prepare('SELECT * FROM locations WHERE id = ?').get(lease.location_id);
  const availableCapacity = location.capacity - location.used_capacity;
